...
export const StudentRegistrationModal: React.FC<StudentRegistrationModalProps> = ({
  ...
  const getDefaultSubjectsForClassArm = (arm: string): string[] => {
    if (arm.startsWith('JSS')) return [...JUNIOR_CURRICULUM];
    if (arm.includes('Science')) return [...SENIOR_SCIENCE_CURRICULUM];
    if (arm.includes('Art')) return [...SENIOR_ART_CURRICULUM];
    if (arm.includes('Commercial')) return [...SENIOR_COMMERCIAL_CURRICULUM];
    return [
      'English Language',
      'General Mathematics',
      'Civic Education',
  ...
  const [enrolledSubjects, setEnrolledSubjects] = useState<string[]>(() => getDefaultSubjectsForClassArm('JSS 1A'));
  const [subjectSearchQuery, setSubjectSearchQuery] = useState('');

  const handleClassChange = (newArm: string) => {
    setClassArm(newArm);
    setEnrolledSubjects(getDefaultSubjectsForClassArm(newArm));
  };

  const toggleEnrolledSubject = (subject: string) => {
    setEnrolledSubjects((prev) =>
      prev.includes(subject) ? prev.filter((s) => s !== subject) : [...prev, subject]
    );
  };

  const selectAllRegistrationSubjects = () => {
    setEnrolledSubjects([...ALL_SCHOOL_SUBJECTS]);
  };

  const resetToClassDefaultSubjects = () => {
    setEnrolledSubjects(getDefaultSubjectsForClassArm(classArm));
  };

  const clearAllRegistrationSubjects = () => {
    setEnrolledSubjects([]);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const studentPayload: Partial<StudentProfile> = {
      ...
      photoUrl,
      resultHeld: false,
      customSubjects: enrolledSubjects,
    } as any;

    try {
      const success = await onRegisterStudent(studentPayload);
      if (success) {
        const initialSubjRecords = enrolledSubjects.map((sName) => ({
          code: getSubjectCode(sName, level),
          name: sName,
          homework: 0,
          test1: 0,
          test2: 0,
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-2 sm:p-4 overflow-y-auto">
      <div className="bg-white rounded-3xl w-full max-w-4xl max-h-[92vh] flex flex-col shadow-2xl border border-slate-200 overflow-hidden my-auto animate-in fade-in duration-200">
        {/* Header */}
        <div className="p-5 sm:p-6 bg-slate-900 text-white relative border-b border-slate-800">
          <button
            type="button"
            onClick={onClose}
          ...
          <div className="flex items-center gap-3.5">
            <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-xl bg-slate-800 text-amber-300 border border-slate-700 flex items-center justify-center shrink-0">
            ...
            <div>
              ...
              <h2 className="text-xl sm:text-2xl font-bold font-serif-title tracking-tight mt-1 text-white">
        ...
        {registeredStudent ? (
          ...
          <form onSubmit={handleSubmit} className="flex-1 overflow-hidden flex flex-col">
            ...
            <div className="flex items-center gap-1 border-b border-slate-200 bg-slate-100/70 px-3 sm:px-6 overflow-x-auto text-xs font-semibold py-2 shrink-0 scrollbar-none">
              <button
                type="button"
            ...
            <div className="p-5 sm:p-6 overflow-y-auto space-y-6 flex-1">
              ...
              {activeSection === 'academic' && (
                <div className="space-y-5 animate-in fade-in duration-150">
                  ...
                  {/* 6. SUBJECT ENROLMENT & ELECTIVE ALLOCATION (Supports Geography & Marketing) */}
                  <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-3">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-200 pb-2.5">
                      <div className="flex items-center gap-2.5">
                        <div className="w-8 h-8 rounded-xl bg-blue-100 text-blue-900 border border-blue-200 flex items-center justify-center shrink-0">
                          <BookOpen className="w-4 h-4" />
                        </div>
                        <div>
                          <div className="flex items-center gap-2">
                            <h4 className="text-xs font-bold text-slate-900">
                              Subject Enrolment & Elective Allocation
                            </h4>
                            <span className="px-2 py-0.5 rounded-full text-[10px] font-extrabold bg-blue-900 text-white">
                              {enrolledSubjects.length} Selected
                            </span>
                          </div>
                          <p className="text-[11px] text-slate-500">
                            Configure curriculum subjects for this student. Defaults to {classArm} curriculum.
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <button
                          type="button"
                          onClick={resetToClassDefaultSubjects}
                          className="px-2.5 py-1 bg-white hover:bg-slate-100 text-slate-700 text-[10px] font-bold rounded-lg border border-slate-300 transition-colors flex items-center gap-1 cursor-pointer"
                        >
                          <RotateCcw className="w-3 h-3" />
                          <span>Reset to {classArm} Defaults</span>
                        </button>
                        <button
                          type="button"
                          onClick={selectAllRegistrationSubjects}
                          className="px-2.5 py-1 bg-blue-50 hover:bg-blue-100 text-blue-900 text-[10px] font-bold rounded-lg border border-blue-200 transition-colors cursor-pointer"
                        >
                          Select All ({ALL_SCHOOL_SUBJECTS.length})
                        </button>
                        <button
                          type="button"
                          onClick={clearAllRegistrationSubjects}
                          className="px-2.5 py-1 bg-white hover:bg-rose-50 text-rose-700 text-[10px] font-bold rounded-lg border border-rose-200 transition-colors cursor-pointer"
                        >
                          Clear
                        </button>
                      </div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="relative flex-1">
                        <input
                          ...
                          placeholder="Search subjects (e.g. Geography, Marketing, English, Biology)..."
                          className="w-full pl-3 pr-8 py-1.5 text-xs bg-white border border-slate-300 rounded-xl focus:outline-hidden focus:ring-2 focus:ring-blue-900/20"
                        />
                        {subjectSearchQuery && (
                          <button
                            type="button"
                    ...
                    {/* Subjects Grid */}
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-1.5 max-h-52 overflow-y-auto p-1 bg-white border border-slate-200 rounded-xl">
                      {ALL_SCHOOL_SUBJECTS.filter((s) =>
                        s.toLowerCase().includes(subjectSearchQuery.toLowerCase()) ||
                        getSubjectCode(s).toLowerCase().includes(subjectSearchQuery.toLowerCase())
                      ).map((subj) => {
                        const isEnrolled = enrolledSubjects.includes(subj);
                        const code = getSubjectCode(subj);
                        const category = getSubjectCategory(subj);
                        const isSpecialHighlight = subj === 'Geography' || subj === 'Marketing';

                        return (
                          <button
                            key={subj}
                            type="button"
                            onClick={() => toggleEnrolledSubject(subj)}
                            className={`flex items-start gap-2 p-2 rounded-lg text-left transition-all border cursor-pointer ${
                              isEnrolled
                                ? isSpecialHighlight
                                  ? 'bg-amber-50/80 border-amber-300 ring-1 ring-amber-400/30'
                                  : 'bg-blue-50/70 border-blue-200 ring-1 ring-blue-300/30'
                                : 'bg-slate-50/50 hover:bg-slate-100 border-slate-200 opacity-70 hover:opacity-100'
                            }`}
                          >
                              ...
                              {isEnrolled ? (
                                <CheckCircle2
                                  className={`w-3.5 h-3.5 ${
                                    isSpecialHighlight ? 'text-amber-700' : 'text-blue-900'
                                  }`}
                                />
                              ) : (
                            ...
                            <div className="min-w-0 flex-1">
                              <div className="flex items-center gap-1">
                                ...
                                {isSpecialHighlight && (
                                  <span className="px-1 py-0.2 rounded text-[8px] font-black bg-amber-200 text-amber-900 shrink-0">
                                    NEW
                                  </span>
                                )}
                              </div>
                    ...
                    <div className="flex items-center justify-between text-[10px] text-slate-500 px-1">
                      <span>
                        Students in <strong>{classArm}</strong> will be registered with all checked subjects above.
                      </span>
                      <span className="font-medium text-slate-600">
                        {enrolledSubjects.length} of {ALL_SCHOOL_SUBJECTS.length} active
                      </span>
                    </div>
                  </div>
              ...
              <div className="flex items-center gap-2">
                {activeSection !== 'biodata' && (
                  <button
                    ...
                    className="px-4 py-2 bg-white hover:bg-slate-100 text-slate-700 border border-slate-200 rounded-xl text-xs font-semibold transition-colors cursor-pointer min-h-[38px]"
                  >
                    ← Previous Step
                  </button>
                )}
              ...
              <div className="flex items-center gap-2">
                {activeSection !== 'bursary' ? (
                  <button
                    ...
                    className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-semibold transition-all flex items-center gap-1.5 cursor-pointer shadow-xs min-h-[38px] border border-slate-800"
                  >
                    <span>Next Step</span>
                  ...
                  <button
                    ...
                    className="px-5 sm:px-6 py-2 bg-slate-900 hover:bg-slate-800 text-white rounded-xl text-xs font-semibold transition-all flex items-center gap-2 cursor-pointer shadow-xs disabled:opacity-50 min-h-[38px] border border-slate-800"
                  >
                    {isSubmitting ? (
